...
-   Randomly divide the set of observations into a \alert{training set} and a \alert{validation set}.
...
-   The \alert{validation set error rate can be highly variable} since it depends on which observations are included in the training set versus the validation set.

-   The validation set error rate may \alert{overestimate the test error rate} since the model is being fit on a smaller training set.
...
Leave-one-out cross-validation (LOOCV) follows the same steps as the validation set approach except the \alert{validation set is just one single observation}. Then,
...
-   It has \alert{less bias} and \alert{does not overestimate the test error rate} as much since it is using a lot more observations to train the model.
...
-   The \alert{computational time/effort for $k$-fold CV for $k < n$ is less} since we are fitting fewer models in the process.

-   \alert{LOOCV is less biased} in its estimation of the test error rate since it trains the model on more observations.

\-\alert{LOOCV has a test error estimate that has higher variance} than $k$-fold CV ($k < n$)
...
\alert{The bootstrap can be used to quantify the uncertainty of an estimate} in the following way:
