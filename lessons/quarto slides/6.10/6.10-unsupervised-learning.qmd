---
title: "6.10 Unsupervised Learning"
...
-   For **supervised learning** we usually have a set of features $X_1, \dots, X_p$ and a response $Y$ measured on $n$ observations. The goal is to \alert{predict $Y$ using $X_1, \dots, X_p$}.

-   For **unsupervised learning** we have a set of features $X_1, \dots, X_p$ and measured on $n$ observations with no response variable. The goal is to \alert{gain information about the features $X_1, \dots, X_p$}.
...
More formally, the first principal loading vector $\phi_{1}=\left(\phi_{11} , \phi_{21} , \ldots , \phi_{p 1}\right)$ solves the following problem: $$\operatorname{maximize}_{\phi_{11}, \ldots, \phi_{p 1}}\left\{\frac{1}{n} \sum_{i=1}^{n}\left(\sum_{j=1}^{p} \phi_{j 1} x_{i j}\right)^{2}\right\} \text{ subject to } \sum_{j=1}^{p} \phi_{j 1}^{2}=1$$
...
::: columns
::: {.column width="60%"}
![](images/first_component.png){fig-align="center" width="320"}
:::
...
::: columns
::: {.column width="50%"}
![](images/second_component3D.png){fig-align="center" width="281"}
:::
...
::: columns
::: {.column width="60%"}
![](images/PCA.png){fig-align="center" width="280"}
:::
...
\quad $\Rightarrow$ We want to know the **proportion of variance explained** (PVE) by each principal component.
...
-   The **total variance** in a data set is: $$\sum_{j=1}^{p} \operatorname{Var}\left(X_{j}\right)=\sum_{j=1}^{p} \frac{1}{n} \sum_{i=1}^{n} x_{i j}^{2}$$

-   The variance explained by the $m$th principal component is: $$\frac{1}{n} \sum_{i=1}^{n} z_{i m}^{2}=\frac{1}{n} \sum_{i=1}^{n}\left(\sum_{j=1}^{p} \phi_{j m} x_{i j}\right)^{2}$$

-   The PVE of the $m$th principal component is: $$\frac{\sum_{i=1}^{n} z_{i m}^{2}}{\sum_{j=1}^{p} \sum_{i=1}^{n} x_{i j}^{2}}=\frac{\sum_{i=1}^{n}\left(\sum_{j=1}^{p} \phi_{j m} x_{i j}\right)^{2}}{\sum_{j=1}^{p} \sum_{i=1}^{n} x_{i j}^{2}}$$
...
::: columns
::: {.column width="70%"}
![](images/scaled_sunscaled.png){fig-align="center" width="400"}
:::
...
-   For a $n \times p$ data matrix $\mathbf{X}$ there are $\min(n-1, p)$ distinct principal components.
...
::: columns
::: {.column width="50%"}
![](images/scree_plot.png){fig-align="center" width="261"}
:::
...
Matrix completion works by \alert{simultaneously estimating the missing values and solving the principal components} iteratively. We will not cover the algorithm but if you are interested see section 12.3 of ISLR2.
...
Want to \alert{choose clusters that minimize the \textbf{within-cluster variation}} $W(C_k)$ for all clusters $C_k$.
...
Within-Cluster variation for the $k$th cluster is the \alert{sum of the pairwise squared Euclidean distances between each observation in the $k$th cluster, divided by the number of observations in the cluster} ($|C_k|$). That is, $$W\left(C_{k}\right)=\frac{1}{\left|C_{k}\right|} \sum_{i, i^{\prime} \in C_{k}} \sum_{j=1}^{p}\left(x_{i j}-x_{i^{\prime} j}\right)^{2}$$

The **Euclidean distance** is a way to measure distance in $p$-dimensional space. Suppose we have $\mathbf{x} = (x_1, \dots, x_p)$ and $\mathbf{y} = (y_1, \dots, y_p)$, then the Euclidean distance is $$d(\mathbf{x}, \mathbf{y}) = \sqrt{(x_1 - y_1)^2 + \cdots + (x_p - y_p)^2}$$ Note that we use the squared Euclidean distance in $W(C_k)$ so we remove the square-root.
...
::: columns
::: {.column width="60%"}
![](images/Local-Global-Optimum.png){fig-align="center" width="300"}
:::
...
::: columns
::: {.column width="50%"}
![](images/dendrogram.png){fig-align="center" width="173"}
:::
...
::: columns
::: {.column width="50%"}
![](images/small_dendogram.png){fig-align="center" width="302"}
:::
...
::: columns
::: {.column width="50%"}
![](images/hierarchical_cluster.png){fig-align="center" width="300"}
:::
...
1.  Compute all pairwise dissimilarities between the observations in cluster A and cluster B.
...
::: columns
::: {.column width="70%"}
![](images/hierarchical_alg1.png){fig-align="center" width="410"}
:::
...
::: columns
::: {.column width="70%"}
![](images/hierarchical_alg2.png){fig-align="center" width="410"}
:::
...
![](images/linkage_choice.png){fig-align="center" width="345"}
...
\quad James, Gareth, et al. "Unsupervised Learning." An Introduction to Statistical Learning:

\quad with Applications in R, 2nd ed., Springer, 2021.

Local optima photo:

\quad By Christoph Roser at AllAboutLean.com under the free CC-BY-SA 4.0 license.
