---
title: "6.5: Linear Model Selection and Regularization"
...
-   Improved \alert{model interpretability} by removing irrelevant variables, thereby reducing the complexity of the model.

-   Improved \alert{prediction accuracy} by a reduction in the risk of overfitting and the variance in the fitted coefficients.
...
-   \alert{Fit all possible models that contain $k$ predictors.} (i.e. if $k = 2$ fit a separate model for each possible combination of two predictors)

-   \alert{Pick the best model from the set of models with $k$ predictors} and call it $\mathcal{M}_k$. The best model is the one with the smallest residual sum of squares (RSS) or largest $R^2$.

2.  \alert{Select the best model} from the set of $\mathcal{M}_{0}, \ldots, \mathcal{M}_{p}$ where $\mathcal{M}_{0}$ contains no predictors and simply predicts the sample mean for each observation.
...
-   \alert{Indirectly estimate the test error} by adjusting the training error to include a measure of bias due to overfitting.

-   \alert{Directly estimate the test error} using a a validation set approach or cross-validation.
...
The AIC is usually used for models fit with maxiumum likelihood but in the case of a linear model with Gaussian errors, maximum likelihood and least squares are the same thing. $$\mathrm{AIC}=\frac{1}{n}\left(\mathrm{RSS}+2 d \hat{\sigma}^{2}\right)$$ So for least squares models, $C_p$ and AIC are the same. As a result, we choose the model with the lowest AIC value.
...
-   Ridge regression coefficient estimates are not scale equivalent.
    ...
    -   Always standardize the predictors so that they are on the same scale before performing ride regression $$\tilde{x}_{i j}=\frac{x_{i j}}{\sqrt{\frac{1}{n} \sum_{i=1}^{n}\left(x_{i j}-\bar{x}_{j}\right)^{2}}}$$

------------------------------------------------------------------------

# Lasso

The Lasso, like ridge regression, minimizes a different quantity than least squares regression in order to estimate the regression coefficients. $$\mathrm{RSS}+\lambda \sum_{j=1}^{p}\left|\beta_{j}\right|$$
