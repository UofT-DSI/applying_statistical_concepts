...
_(vi)_. **[1 POINT]** Write code to review the bootstrapped standard deviation (Hint: the function name is `sd`):
```{r}
#code here
```
...
_(viii)_ **[2 POINTS]** Manually compute MSE for the linear model (without LOOCV) that you fit with the `glm` function, in iv. (Hint: recall that MSE is defined as the sum of squared residuals, divided by n-p. You can "look inside" your linear model object to find residual values).  
